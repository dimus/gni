module GNI
  module XML
    ...
    class TcsXmlBuilder
      ...
      def initialize(data_source)
        ...
        @tcs_file.write(XML_HEAD + '<DataSet
      ...
      def make_node(record_data, count)
        ...
        record << (" " * 6) + "<Simple>#{GNI::XML.escape_xml(record_data['dwc:ScientificName'])}</Simple>"
        record << (" " * 6) + "<Rank>#{GNI::XML.escape_xml(record_data['dwc:TaxonRank'])}</Rank>" if record_data.key? 'dwc:TaxonRank'
        record << (" " * 6) + "<ProviderSpecificData>"
        record << (" " * 8) +  "<dwc:Kingdom>#{GNI::XML.escape_xml(record_data['dwc:Kingdom'])}</dwc:Kingdom>" if record_data.key? 'dwc:Kingdom'
        record << (" " * 8) + "<dc:identifier>#{GNI::XML.escape_xml(record_data['dc:identifier'])}</dc:identifier>" if record_data.key? 'dc:identifier'
        record << (" " * 8) + "<dc:source>#{GNI::XML.escape_xml(record_data['dc:source'])}</dc:source>" if record_data.key? 'dc:source'
        record << (" " * 8) + "<dwc:GlobalUniqueIdentifier>#{GNI::XML.escape_xml(record_data['dwc:GlobalUniqueIdentifier'])}</dwc:GlobalUniqueIdentifier>" if record_data.key? 'dwc:GlobalUniqueIdentifier'
        record << (" " * 6) + "</ProviderSpecificData>"
        record << (" " * 4) + "</TaxonName>"
    ...
    #TODO needs refactoring to remove duplication of functionality with TcsXmlBuilder 
  ...
  '
      ...
      name_index_records.each  do |r|
        ...
        tcs += "      <Simple>#{self.escape_xml(r.name)}</Simple>"
        tcs += "      <Rank>#{self.escape_xml(r.rank)}</Rank>" if r.rank
        tcs += "      <ProviderSpecificData>"
        tcs += "        <dc:Kingdom>#{self.escape_xml(r.kingdom.name)}</dc:Kingdom>" if r.kingdom
        tcs += "        <dc:identifier>#{r.local_id}</dc:identifier>" if r.local_id
        tcs += "        <dwc:GlobalUniqueIdentifier>#{self.escape_xml(r.global_id)}</dwc:GlobalUniqueIdentifier>" if r.global_id
        tcs += "      </ProviderSpecificData>"
        tcs += "    </TaxonName>"
      end
      tcs += '
